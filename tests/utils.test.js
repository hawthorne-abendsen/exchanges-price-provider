...
const {getBigIntPrice, getInversedPrice, getVWAP, getMedianPrice} = require('../utils')

function arrToOhlcvs(val) {
...
describe('utils', () => {
    ...
    it('get inversed price for non BigInt', () => {
        ...
        expect(() => getInversedPrice(price, 14)).toThrowError('Price must be BigInt')
    })
    ...
    it('get BigInt price for NaN', () => {
        ...
        expect(() => getBigIntPrice(price, 14)).toThrowError('Price must be a number')
    })

    it('get BigInt price for NaN decimals', () => {
        const price = 1000
        expect(() => getBigIntPrice(price, 'Not a number')).toThrowError('Decimals must be a number')
    })
    ...
    it('get median price', () => {
        const medianPrice = getMedianPrice(arrToOhlcvs([1000000000000000000n, 2000000000000000000n, 3000000000000000000n]))
    ...
    it('get median price for zero', () => {
        const medianPrice = getMedianPrice(arrToOhlcvs([0n, 0n, 0n]))
        expect(medianPrice).toBe(null)
    })

    it('get median price for single', () => {
        const medianPrice = getMedianPrice(arrToOhlcvs([1000000000000000000n]))
    ...
    it('get median price for odd', () => {
        ...
        for (const testCase of testCasses) {
            const medianPrice = getMedianPrice(arrToOhlcvs(testCase.data))
    ...
    it('get median price for even with single non-zero', () => {
        const medianPrice = getMedianPrice(arrToOhlcvs([0n, 0n, 1000000000000000000n, 0n]))
        expect(medianPrice).toBe(1000000000000000000n)
    })

    it('get median price for even with null and undefined', () => {
        const medianPrice = getMedianPrice(arrToOhlcvs([970n, 1010n, 1000n, null, undefined, 0n]))
